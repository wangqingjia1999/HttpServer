...
TEST(base64_tests, encode_and_decode_test)
{
	...
	std::string encoded_string = "QSBqb3VybmV5IG9mIGEgdGhvdXNhbmQgbWlsZXMgYmVna"
	                             "W5zIHdpdGggYSBzaW5nbGUgc3RlcA==";
	...
	{
		EXPECT_EQ(Base64::encode(test_vector.decoded_string, true),
		          test_vector.encoded_string)
		    << "Error at index: " << index;
		EXPECT_EQ(Base64::decode(test_vector.encoded_string, true),
		          test_vector.decoded_string)
		    << "Error at index: " << index;
		++index;
	}
}
...
TEST(base64_tests, encode_and_decode_url_test)
{
	...
	{
		EXPECT_EQ(testVector.encoding,
		          Base64::encode_url(testVector.decoding, false))
		    << "Error at index: " << index;
		EXPECT_EQ(testVector.decoding,
		          Base64::decode_url(testVector.encoding, false))
		    << "Error at index: " << index;
		++index;
	}
}
...
TEST(base64_tests, encode_hex_string_test)
{
	/**
	 * The problem is that:
	 * b37a4f2cc0624f1690f64606cf385945b2bec4ea is byte sequence in the form of
	 * hex, we need convert this kind of string to byte sequence first;
