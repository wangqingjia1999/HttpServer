...
TEST(uri_tests, parse_from_url_string)
{
	Uri uri;
	ASSERT_TRUE(
	    uri.parse_from_string("https://www.github.com/Bitate/httpserver"));
	ASSERT_EQ(uri.get_scheme(), "https");
	ASSERT_EQ(uri.get_host(), "www.github.com");
	ASSERT_EQ(uri.get_path(),
	          (std::vector<std::string>{"Bitate", "httpserver"}));
...
TEST(uri_tests,
     parse_from_string_bad_port_number_starts_numeric_ends_alphabetic)
{
	Uri uri;
	ASSERT_FALSE(
	    uri.parse_from_string("http://www.example.com:8080spam/foo/bar"));
}
...
TEST(uri_tests, parse_bad_port_number_starts_alphabetic_and_ends_numeric)
{
	Uri uri;
	ASSERT_FALSE(
	    uri.parse_from_string("http://www.example.com:spam8080/foo/bar"));
}
...
TEST(uri_tests, parse_bad_port_number_too_big)
{
	...
	ASSERT_FALSE(
	    uri.parse_from_string("http://www.example.com:655360/foo/bar"));

	ASSERT_FALSE(
	    uri.parse_from_string("http://www.example.com:6553600/foo/bar"));
}
...
TEST(uri_tests, parse_relative_path_and_absolute_path)
{
	...
	for (const auto& test_vector : test_vectors)
	{
		...
		ASSERT_EQ(test_vector.is_relative_reference,
...
TEST(uri_tests, parse_relative_and_non_relative_path)
{
	...
	const std::vector<TestVector> test_vectors{
	    ...
	    {"/", false},
	    {"foo", true},
	    // An empty Uri string IS a valid "relative reference" with an empty
	    // path.
	    {"", true},
	};
	...
	for (const auto& test_vector : test_vectors)
	{
		Uri uri;
		ASSERT_TRUE(uri.parse_from_string(test_vector.uri)) << index;
		ASSERT_EQ(test_vector.has_relative_path, uri.has_relative_path())
		    << index;
		++index;
	}
}
...
TEST(uri_tests, parse_query_and_fragment_elements)
{
	...
	const std::vector<TestVector> test_vectors{
	    ...
	    {"http://www.example.com?earth?day#bar", "www.example.com", "earth?day",
	     "bar"},
	    {"http://www.example.com/spam?foo#bar", "www.example.com", "foo",
	     "bar"},
...
TEST(uri_tests, parse_query_parameters)
{
	...
	std::vector<TestVector> test_vectors{
	    ...
	    {"name=Tom&age=22&country=China",
	     {{"age", "22"}, {"country", "China"}, {"name", "Tom"}}},
	    {"name=&age=22&=China", {{"age", "22"}}},
	    {"Tom&=22&&&country=China", {{"country", "China"}}},
	    {"country=China&=22&&&", {{"country", "China"}}}};

	size_t index = 0;
	for (const auto& test_vector : test_vectors)
	{
		Uri uri;
		ASSERT_TRUE(uri.parse_query_parameters(test_vector.query_string))
		    << index;
		ASSERT_EQ(uri.get_query_paramters(), test_vector.query_parameters)
		    << index;
		++index;
	}
}