...
{
	Utf8::Utf8 utf8;
	std::vector<uint8_t> expectedEncoding{0x41, 0xE2, 0x89, 0xA2,
	                                      0xCE, 0x91, 0x2E};
...
{
	Utf8::Utf8 utf8;
	const auto actualDecoding = utf8.Decode(std::vector<uint8_t>{
	    0xE6, 0x97, 0xA5, 0xE6, 0x9C, 0xAC, 0xE8, 0xAA, 0x9E});
	ASSERT_EQ((std::vector<Utf8::UnicodeCodePoint>{0x65E5, 0x672C, 0x8A9E}),
	          actualDecoding);
}
...
TEST(Utf8Tests, UnexpectedContinuationBytes)
{
	...
	ASSERT_EQ(
	    ...
	    utf8.Decode(
	        std::vector<uint8_t>{0x41, 0xE2, 0x89, 0xA2, 0x91, 0x2E})); // A≢�.
}
...
TEST(Utf8Tests, DecodeBreakInSequence)
{
	...
	ASSERT_EQ(
	    (std::vector<Utf8::UnicodeCodePoint>{0x0041, 0x2262, 0xFFFD, 0x002E}),
	    utf8.Decode(
	        std::vector<uint8_t>{0x41, 0xE2, 0x89, 0xA2, 0xCE, 0x2E})); // A≢�.
}
...
TEST(Utf8Tests, RejectOverlongSequences)
{
	...
	{
		Utf8::Utf8 utf8;
		ASSERT_EQ((std::vector<Utf8::UnicodeCodePoint>{0xFFFD}),
...
TEST(Utf8Tests, StumpOfTreeDecodedInTwoParts)
{
	...
	ASSERT_EQ((std::vector<Utf8::UnicodeCodePoint>{}),
	          utf8.Decode(firstHalfOfEncoding));
	ASSERT_EQ((std::vector<Utf8::UnicodeCodePoint>{0x233B4}), // 𣎴
