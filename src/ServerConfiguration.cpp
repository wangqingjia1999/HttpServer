...
namespace
{
	...
	const std::string resource_directory_path = {root_directory_path +
	                                             "resource/"};

	const std::string asset_directory_path = {resource_directory_path +
	                                          "assets/"};
	...
	const std::string database_file_path = {data_storage_directory_path +
	                                        "data.db"};
} // namespace
...
namespace
{
	const std::string default_home_html{
	    ...
	    "<p>Find real-life English sentences taken from reliable news websites "
	    "to facilitate English learning.</p>"
	    "<p>The keywords inside research result will be highlighted to save "
	    "your time. :^)"
	    "</span>"

	    "<div id=\"search-bar-wrap\">"
	    "<form role=\"search\" method=\"GET\" action=\".\">"
	    "<input type=\"search\" id=\"search-bar-input\" placeholder=\"Type a "
	    "word or phrase here\" name=\"q\">"
	...
	const std::string default_home_css{"body {"
	                                   "height: 100%;"
	                                   "margin: 0px;"
    ...
    , m_log_directory_path{log_directory_path}
{
	...
	create_file_if_not_exist(resource_directory_path + "index.html",
	                         default_home_html);
	create_file_if_not_exist(css_directory_path + "HomePage.css",
	                         default_home_css);
}
